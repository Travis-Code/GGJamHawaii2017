...
Parkour.GameState = {
    ...
    create: function() {
        ...
        this.text = this.game.add.text(250, this.game.world.centerY-50, "TSUNAMI\nINCOMING\nRUN");
    ...
    update: function() {
        ...
        this.paralax1.x = this.game.camera.x * 0.2;
        this.paralax1.y = this.game.camera.y * 0.2 - 300; //move it down a few pixels to account for the missing pixels when moving with camera
        ...
        this.game.physics.arcade.overlap(this.player, this.tsunami);
        this.game.physics.arcade.overlap(this.pipeWarp, this.tsunami);
        ...
        if (this.playerAlive) {
            this.game.physics.arcade.overlap(this.player, this.tsunami, null, function(p, g) {
                this.game.bgMusic.stop();
                console.log('tsunami hit you!');
                Parkour.game.state.start('Game');
                this.tsunami.destroy();
            }, this);
        }
    ...
    loadLevel: function() {
        ...
        this.paralax1 = this.game.add.tileSprite(0, 800, 6300, 816, 'clouds');
        this.paralax2 = this.game.add.tileSprite(0, 400, 6300, 816, 'backgroundCity');
        ...
        var monster = new Parkour.Monsters(this.game, 700, this.game.height / 2, "cat", undefined, 800, 90);
        monster.animations.add("catWalk", [0, 1], 4, true);
        monster.play("catWalk");
        //sampleCat.tint =  this.tintCatColor;
        this.monsters.add(monster);
        //this.monsters.scale.setTo(0.3);
        var monster = new Parkour.Monsters(this.game, 900, this.game.height / 2, "cat", undefined, 1000, 600);
        monster.animations.add("catWalk", [0, 1], 4, true);
        monster.play("catWalk");
        //sampleCat.tint =  this.tintCatColor;
        this.monsters.add(monster);
        //this.monsters.scale.setTo(0.3);
        var monster = new Parkour.Monsters(this.game, 6500, this.game.height / 2, "cat", undefined, 7000, 6000);
        monster.animations.add("catWalk", [0, 1], 4, true);
        monster.play("catWalk");
        //sampleCat.tint =  this.tintCatColor;
        this.monsters.add(monster);
